import i18n, { getCurrentAppLanguage } from "@/i18n";
...
export async function searchWithGoogle(
  ...
  url.searchParams.set("language", getGoogleLanguage());
  ...
  if (data.status !== "OK" && data.status !== "ZERO_RESULTS") {
    throw new Error(
      data.error_message || i18n.t("location:googleSearchFailed"),
    );
  }
...
export async function getGooglePlaceDetails(
  ...
  if (data.status !== "OK" || !data.result?.geometry?.location) {
    throw new Error(data.error_message || i18n.t("location:detailsFailed"));
  }
...
export async function reverseWithGoogle(
  ...
  if (data.status !== "OK" || !data.results?.[0]) {
    throw new Error(data.error_message || i18n.t("location:reverseFailed"));
  }
