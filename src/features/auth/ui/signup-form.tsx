...
interface SignupFormProps {
  ...
  onFocusChange: (field: "name" | "email" | "password" | null) => void;
  onSubmit: () => void;
  onModeChange: () => void;
}
...
const getIconColor = (field: string, focusedField: string | null) =>
  ...
  return (
    <View className="px-6 pb-6 pt-7">
      ...
      <Text className="mt-3 font-body text-base leading-6 text-brand-neutral">
        Hesabını oluştur ve ilerlemeni tek yerde topla.
      </Text>
      ...
      >
        <FontAwesome6
          size={18}
          name="user"
          color={getIconColor("name", focusedField)}
        />
        <TextInput
          value={name}
      ...
      <Text className="mt-4 text-center font-mono text-xs text-brand-neutral">
        Kayıt sırasında ad soyad bilgisi API'ye gönderilir.
      </Text>
    </View>
  );
}
